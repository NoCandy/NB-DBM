...
MHT.Version = "1.6.3";
MHT.Author = "NB-Bodil"; -- Original by Arta, kc10577
...
MHT:AddBarOption("Rage Winterchill in");
MHT:AddBarOption("Anetheron in");
MHT:AddBarOption("Kazrogal in");
MHT:AddBarOption("Azgalor in");
...
function MHT:OnEvent(event, arg1)
	...
	elseif event == "UPDATE_WORLD_STATES" and GetRealZoneText() == DBM_MOUNT_HYJAL then
		...
		if wave > lastwave then
			...
			if boss == 1 or boss == 2 then	
				...
				elseif MHT.Options.Mobs and boss == 2 then
					...
					elseif wave == 7 then
						self:Announce(string.format(DBM_MHT_WAVE_INC_WARNING4, wave, 2, DBM_MHT_GHOUL, 4, DBM_MHT_FIEND, 4, DBM_MHT_ABOMINATION, 4, DBM_MHT_BANSHEE), 1);
					elseif wave == 8 then
			...
			if wave == 8 and boss == 1 then
				self:StartStatusBarTimer(timer, "Rage Winterchill in", "Interface\\Icons\\Ability_creature_cursed_05");
				self:ScheduleSelf(timer-10, "WaveSoon");
			elseif wave == 8 and boss == 2 then
				self:StartStatusBarTimer(timer, "Anetheron in", "Interface\\Icons\\Spell_shadow_curseoftounges");
				self:ScheduleSelf(timer-10, "WaveSoon");
			elseif wave == 8 and boss == 3 then
				self:StartStatusBarTimer(timer, "Kazrogal in", "Interface\\Icons\\Spell_shadow_curseoftounges");
				self:ScheduleSelf(timer-10, "WaveSoon");
			elseif wave == 8 and boss == 4 then
				self:StartStatusBarTimer(timer, "Azgalor in", "Interface\\Icons\\Spell_shadow_curseoftounges");
				self:ScheduleSelf(timer-10, "WaveSoon");			
			else	
