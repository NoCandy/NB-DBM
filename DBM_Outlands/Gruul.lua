...
Gruul.Version	= "1.1";
Gruul.Author	= "NB-Bodil"; -- Original by Tandanu
Gruul.Grows		= 0;
...
Gruul:AddOption("SilenceWarn", true, DBM_GRUUL_SILENCE_OPT);
...
function Gruul:OnCombatStart(delay)
	self.Grows = 0;	
	--self:ScheduleSelf(104 - delay, "SilenceSoon"); Random on nightbane
	self:StartStatusBarTimer(108 - delay, "Silence on CD", "Interface\\Icons\\Spell_Holy_ImprovedResistanceAuras");	
	self:ScheduleSelf(55 - delay, "SlamSoon");
	self:StartStatusBarTimer(60 - delay, "Ground Slam", "Interface\\Icons\\Spell_Nature_ThunderClap");	
...
function Gruul:OnEvent(event, arg1)
	if event == "CHAT_MSG_MONSTER_EMOTE" then
		...
		elseif arg1 == DBM_GRUUL_EMOTE_SHATTER then
			...
			self:ScheduleSelf(50, "SlamSoon");
			self:EndStatusBarTimer("Ground Slam");
			self:StartStatusBarTimer(55, "Ground Slam", "Interface\\Icons\\Spell_Nature_ThunderClap");
		end
	elseif event == "SPELL_AURA_APPLIED" then
		...
		elseif arg1.spellId == 36297 then
			...
			--self:UnScheduleSelf("SilenceSoon"); Random on nightbane
			self:EndStatusBarTimer("Silence on CD");
			--self:ScheduleSelf(34, "SilenceSoon");
			self:StartStatusBarTimer(27, "Silence on CD", "Interface\\Icons\\Spell_Holy_ImprovedResistanceAuras");
		end
	elseif event == "SPELL_CAST_START" then
		if arg1.spellId == 33525 then
			if self.Options.ShatterWarn then
