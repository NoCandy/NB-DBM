...
Mag.Version		= "1.1";
Mag.Author		= "NB-Bodil"; -- Original by Tandanu
...
Mag:AddOption("WarnHeal", false, DBM_MAG_OPTION_2);
...
function Mag:OnEvent(event, arg1)
	...
	elseif event == "CHAT_MSG_MONSTER_YELL" then
		if string.find(arg1, DBM_MAG_YELL_PHASE2) then -- to support stupid german localization :(
	...
	elseif event == "CHAT_MSG_RAID_BOSS_EMOTE" then
		if arg1 == DBM_MAG_EMOTE_NOVA then
			...
			self.LastBlastNova = GetTime();
			self:StartStatusBarTimer(60, "Blast Nova", "Interface\\Icons\\Spell_Fire_SealOfFire");
			self:ScheduleSelf(54, "NovaWarn");
			self:ScheduleSelf(75, "Checkback");
		end
	elseif event == "Checkback" and arg1 then
		if (GetTime() - self.LastBlastNova) > 70 then
			self:StartStatusBarTimer(45, "Blast Nova", "Interface\\Icons\\Spell_Fire_SealOfFire");
		end	
