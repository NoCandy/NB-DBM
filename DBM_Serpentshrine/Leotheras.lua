...
Leotheras.Author			= "NB-Bodil"; -- Original by Tandanu
...
function Leotheras:OnCombatStart(delay)
	...
	self:ScheduleSelf(50, "PhaseWarn");
	self:StartStatusBarTimer(55, "Demon Form", "Interface\\Icons\\Spell_Shadow_Metamorphosis");
	
	self:StartStatusBarTimer(10, "Next Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
	self:ScheduleSelf(5, "WhirlWarn2");	
end
...
--			self:ScheduleSelf(10, "DemonsSoon");
		...
		elseif string.find(arg1, DBM_LEO_YELL_SHADOW) then
			self.Phase = "normal";
...
function Leotheras:OnSync(msg)
	...
	elseif msg == "FadeWW" and self.Phase ~= "demon" then
		...
		if not self:GetStatusBarTimerTimeLeft("Demon Form") or self:GetStatusBarTimerTimeLeft("Demon Form") > 13 then
			self:StartStatusBarTimer(15, "Next Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
