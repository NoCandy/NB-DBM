...
Alar.Version	= "1.1";
Alar.Author		= "NB-Bodil"; --Original by Tandanu
...
function Alar:OnCombatStart(delay)	
	...
	flying = false;
	langError = false;
	self:StartStatusBarTimer(34 - delay, "Next Platform", "Interface\\AddOns\\DBM_API\\Textures\\CryptFiendUnBurrow");
...
function Alar:OnSync(msg)
	if msg == "Rebirth" and not self:IsWipe() and self.InCombat then
		...
		self:ScheduleSelf(53, "MeteorSoon");
		self:StartStatusBarTimer(60, "Meteor", "Interface\\Icons\\Spell_Fire_Fireball02");
	...
	elseif msg == "Meteor" and (GetTime() - lastMeteor) > 30 and self.InCombat then
		lastMeteor = GetTime();
		if self.Options.Meteor then
			self:Announce(DBM_ALAR_WARN_METEOR, 4);
		end
		self:ScheduleSelf(58.5, "MeteorSoon");
		self:StartStatusBarTimer(66.5, "Meteor", "Interface\\Icons\\Spell_Fire_Fireball02");
		self:StartStatusBarTimer(10.5, "TOUCHDOWN!", "Interface\\Icons\\Spell_Fire_Flare");
