...
Kael.Version	= "1.4";
Kael.Author		= "NB-Bodil"; --Original by Tandanu
...
Kael:AddOption("ThalaWhisper", false, DBM_KAEL_OPTION_WHISPER_P1);
...
Kael:AddOption("TimerConflag2", true, DBM_KAEL_OPTION_CONFLAGTIMER2);
Kael:AddOption("WarnFear", false, DBM_KAEL_OPTION_FEAR);
Kael:AddOption("WarnFearSoon", false, DBM_KAEL_OPTION_FEARSOON);
...
Kael:AddBarOption("Gravity Lapse Duration")
Kael:AddBarOption("Next Gravity Lapse")

Kael:RegisterCombat("YELL", DBM_KAEL_YELL_PHASE1, nil, nil, nil, 60);

function Kael:OnCombatStart(delay)
	phase = 1
	if self.Options.WarnPhase then
		self:Announce(DBM_KAEL_WARN_PHASE1, 3);
	end
	self:StartStatusBarTimer(30 - delay, "Thaladred", "Interface\\Icons\\Spell_Nature_WispSplode");
	MCTargets = {};
	MCIcons = {
		[1]	= false,
		[2] = false,
...
function Kael:OnEvent(event, arg1)
	if event == "CHAT_MSG_MONSTER_EMOTE" and arg1 then
		self:ScheduleSelf(0.5, "GazeDelay");
	elseif event == "GazeDelay" then
		self:SendSync("ThalaTarget");
	...
	elseif event == "WarnPhase3" then
		if self.Options.WarnPhase then
			self:Announce(DBM_KAEL_WARN_PHASE3, 3);
		end
	elseif event == "Phase3" then
		self:StartStatusBarTimer(170, "Phase 4", "Interface\\Icons\\Spell_Shadow_BloodBoil");
		if self.Options.RangeCheck then
			DBM_Gui_DistanceFrame_Show();
		end
...
function Kael:OnSync(msg)
	if msg == "ThalaTarget" then
		local target
		for i = 1, GetNumRaidMembers() do
			if UnitName("raid"..i.."target") == DBM_KAEL_THALA_NAME then
		...
		if target then
			...
			if self.Options.WarnThalaTarget then
				self:Announce(string.format(DBM_KAEL_WARN_THALADRED_TARGET, target), 2);
			end	
			...
			self:StartStatusBarTimer(8, "Gaze: "..target, "Interface\\Icons\\Spell_Fire_BurningSpeed");
		end	
	...
	elseif msg == "Phase2" then
		...
		self:StartStatusBarTimer(128, "Phase 3", "Interface\\Icons\\Spell_Shadow_AnimateDead");
		self:ScheduleSelf(123, "WarnPhase3");
		if self.Options.ShowFrame and DBMGui then
			if weaponFrame then
				weaponHealth = {
					[1] = 100,
					[2] = 100,
				...
				if (not weaponFrame) then 
					self:AddMsg("Error while creating frame...");
					return;
				end
				for i = 1, 7 do
	...
	elseif msg == "Phase3" then
		...
		if self.Options.ShowAddFrame and DBMGui then
			if addFrame then
				addHealth = {
					[1] = 100,
					[2] = 100,
	...
	elseif msg == "Phase4" then
		...
		phase5 = false;
		if self.Options.WarnPhase then
			self:Announce(DBM_KAEL_WARN_PHASE4, 3);
		end
	...
	elseif msg == "Phase5" then
		...
		phase5 = true;
		if self.Options.WarnPhase then
			self:Announce(DBM_KAEL_WARN_PHASE5, 3);
		end
		gravityLapse = false;
	...
	elseif msg == "Barrier" then
		...
		if not phase5 then
			self:ScheduleSelf(51, "BarrierWarn");
			self:StartStatusBarTimer(56, "Next Shock Barrier", "Interface\\Icons\\Spell_Nature_LightningShield");
		end
	...
	elseif msg == "BarrierDown" then
		...
		self:EndStatusBarTimer("Shock Barrier");
	...
	elseif msg == "GravityLapse" then
		...
		self:StartStatusBarTimer(32.5, "Gravity Lapse Duration", "Interface\\Icons\\Spell_Magic_FeatherFall");
		self:ScheduleSelf(32.5, "GravityLapseEnd");
		self:ScheduleSelf(24.5, "GravityEndWarn");
	end
end
		...
		for i = 1, GetNumRaidMembers() do
			if UnitName("raid"..i.."target") then
				if DBM_KAEL_WEAPONS[UnitName("raid"..i.."target")] then
					weaponHealth[DBM_KAEL_WEAPONS[UnitName("raid"..i.."target")]] = math.floor(UnitHealth("raid"..i.."target")/(UnitHealthMax("raid"..i.."target")/100));

				elseif DBM_KAEL_ADVISORS[UnitName("raid"..i.."target")] then
					addHealth[DBM_KAEL_ADVISORS[UnitName("raid"..i.."target")]] = math.floor(UnitHealth("raid"..i.."target")/(UnitHealthMax("raid"..i.."target")/100));
				elseif UnitName("raid"..i.."target") == DBM_KAEL_EGG then
					egg = UnitHealth("raid"..i.."target")/(UnitHealthMax("raid"..i.."target")/100);
				end
			end
		end
		self:UpdateHealth();
