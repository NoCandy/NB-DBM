...
VoidReaver.Version		= "1.1";
VoidReaver.Author		= "NB-Bodil"; --Original by Tandanu
...
VoidReaver:AddOption("WarnPounding", false, DBM_VOIDREAVER_OPTION_WARN_POUNDING);
VoidReaver:AddOption("WarnPoundingSoon", false, DBM_VOIDREAVER_OPTION_WARN_POUNDINGSOON);
...
function VoidReaver:OnArcaneOrb(target)
	...
	if target == UnitName("player") then
		...
		if self.Options.SoundWarning then
			...
			--PlaySoundFile("Sound\\Creature\\HoodWolf\\HoodWolfTransformPlayer01.wav"); to spammy
		end 
	end
